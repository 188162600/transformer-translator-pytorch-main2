...
def load_trails(training_config):
    ...
    if os.path.exists(training_config['step_losses_pth']):
        with open(training_config['step_losses_pth'], 'r',encoding='UTF-8') as file:
            step_losses = json.load(file)
            file.close()

    train_losses = list()
    if os.path.exists(training_config['train_losses_pth']):
        with open(training_config['train_losses_pth'], 'r',encoding='UTF-8') as file:
            train_losses = json.load(file)
            file.close()
    
    eval_losses = list()
    if os.path.exists(training_config['eval_losses_pth']):
        with open(training_config['eval_losses_pth'], 'r',encoding='UTF-8') as file:
            eval_losses = json.load(file)
            file.close()

    train_accuracy = list()
    if os.path.exists(training_config['train_accuracy_pth']):
        with open(training_config['train_accuracy_pth'], 'r',encoding='UTF-8') as file:
            train_accuracy = json.load(file)
            file.close()
    
    eval_accuracy = list()
    if os.path.exists(training_config['eval_accuracy_pth']):
        with open(training_config['eval_accuracy_pth'], 'r',encoding='UTF-8') as file:
...
def save_trails(training_config, step_losses, train_losses, eval_losses, train_accuracy, eval_accuracy):
    with open(training_config['step_losses_pth'], 'w',encoding='UTF-8') as file:
        json.dump(step_losses, file)
        file.close()

    with open(training_config['train_losses_pth'], 'w',encoding='UTF-8') as file:
        json.dump(train_losses, file)
        file.close()
    
    with open(training_config['eval_losses_pth'], 'w',encoding='UTF-8') as file:
        json.dump(eval_losses, file)
        file.close()

    with open(training_config['train_accuracy_pth'], 'w',encoding='UTF-8') as file:
        json.dump(train_accuracy, file)
        file.close()
    
    with open(training_config['eval_accuracy_pth'], 'w',encoding='UTF-8') as file:
